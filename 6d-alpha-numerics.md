...
### 5.8 Acquis empiriques post-A.5 et §5.7

**Acquis A.5 (sweep β ∈ {30, 45, 60, 80, 100}, γ=1, σ_0=1.8)** :
Les 5 valeurs satisfont les filtres durs spec et atteignent stationnarité
à machine precision (rel_norm(dψ/dt), rel_norm(dh/dt active) ∈ [10⁻¹³, 10⁻¹⁶]).
**Tous les sweeps donnent L2_rel ≈ 0 et corr ≈ 1** pour le test §5.

**Lecture critique** : ce résultat n'est PAS une validation forte de
la co-production MCQ. À stationnarité exacte du système d'ODE local
`∂_t h = -β·ψ·h + γ·h·(1-h/h₀) = 0`, sur cellules h > 0 :
...
**La formule §5 est l'identité algébrique du point fixe couplé local.**
Elle est satisfaite par construction dès que le moteur converge vers
ce point fixe, quel qu'il soit. §5 mesure donc la **cohérence
numérique du moteur Euler avec son point fixe analytique**, pas
encore une co-production MCQ non-triviale.

**Acquis §5.7 famille B (β ∈ {45, 60, 80}, familles A/B1/B2/B3)** :

Trajectoires comparées pour 3 initialisations alternatives :
- B1 : gaussienne décentrée (1, 2, 2), σ_0=1.8
- B2 : bimodale, deux pics étroits σ_0=1.0 → ψ_max_init élevé
- B3 : uniforme ψ = 1/N_total (contrôle chemin minimal)

**Résultat empirique majeur — multi-bassins d'attraction** :

| β | D_h_final(A↔B1) | D_h_final(A↔B2) | D_h_final(A↔B3) |
|---|---|---|---|
| 45 | 9.5e-07 (même) | **0.163** (différent) | 4.8e-15 (même) |
| 60 | 1.6e-05 (même) | **0.295** (différent) | 1.2e-14 (même) |
| 80 | **0.233** (différent) | **0.729** (différent) | 0.090 (limite) |

**B2 atteint un attracteur structurellement différent de A** pour
tous les β testés. La formule §5 reste satisfaite **dans chaque
bassin séparément** (corr ≈ 1 à stationnarité), mais les bassins
sont distincts au sens de l'état final h.

**Mécanisme séparateur identifié** :

Le critère qui détermine quel bassin est atteint n'est pas `β/γ` global
mais **`β·ψ_max_init / γ` local** combiné à la capacité de la diffusion
à uniformiser ψ avant que h ne collapse :

- `β·ψ_max_init/γ ≲ 1` modérément ET diffusion suffisamment rapide
  → **bassin uniforme** : h converge vers `h₀·(1-β·ψ_∞/γ)` partout,
  pas de collapse local persistant
- `β·ψ_max_init/γ ≫ 1` et collapse local plus rapide que
  l'uniformisation → **bassin collapse partiel persistant** :
  cellules verrouillées DYNAMICALLY_LOCKED_DEEP (mécanisme 4a-η)
  permanentes dans la fenêtre simulée

C'est **la même bifurcation locale qu'en 4a-γ et 4a-η**, mais opérant
maintenant au niveau de la **sélection du bassin global** depuis la
condition initiale.

**Distinction structurelle à inscrire** :

> **§5 valide la relation locale du bassin atteint** (tautologie
> algébrique du point fixe).
> **§5.7 famille B révèle la structure globale des bassins**
> (résultat empirique non-trivial).

**Trois résultats à conserver pour 6d-β** :

(1) **Tautologie locale au point fixe** : `ψ = (γ/β)(1-h/h₀)`
satisfaite par construction sur cellules h > 0 à stationnarité.

(2) **Géométrie de convergence non-triviale** : même quand A et B1
convergent vers le même attracteur, `AUC_Dh` reste significatif
(7.7 à 156). La mémoire morphologique de la trajectoire n'est pas
effacée par la convergence finale.

(3) **Multi-bassins empiriques** : B2 démontre l'existence d'au
moins deux bassins distincts. Le système n'a pas un unique attracteur
universel — la condition initiale détermine le bassin.

**Conséquence méta-méthodologique** : la mémoire MCQ pertinente
n'est pas dans l'équation locale (qui est tautologique au point fixe)
mais dans **le chemin d'accès au bassin** et **la structure des
bassins**. C'est une signature exploitable pour τ', 𝕋(t), RR/RR²,
MCQᴺ — mais qui requiert des observables différentes de la simple
relation `ρ_obs vs ρ_pred`.

**Caveat (rappel §0quater)** : ces résultats sont des propriétés du
schéma discret dans la fenêtre temporelle simulée. "Multi-bassins
empiriques" ≠ "multi-attracteurs structurellement garantis MCQ". La
distinction reste valide tant que des observables non-locales (𝒞_T,
RR/RR²) n'ont pas été testées en 6d-β.
...
### 6.6 Acquis empiriques post-§5.7

Le cadrage de §6 a évolué post-§5.7 : §6 n'est plus une simple clôture
de validation numérique mais un **test de viabilité ouverte**. Le
moteur peut PASS sur garanties tout en produisant des régimes
localement non transformables (B2 §5.7).

**Classification à deux couches introduite** :

- `ENGINE_GUARANTEE_STATUS` : le moteur respecte-t-il les garanties
  par construction ?
- `REGIME_VIABILITY_STATUS` : le régime atteint préserve-t-il la
  transformabilité locale ?

**Résultats §6 (β=60, γ=1, D=0.1, h₀=1.0)** :

| Test | ENGINE_GUARANTEE | REGIME_VIABILITY |
|---|---|---|
| §6.1 D_min | PASS | LOCAL_TRANSPORT_CLOSURE_IN_B2_ONLY |
| §6.2 perturbation bornée | PASS | ABSORBING_BASIN_EMPIRICAL_B2 |
| §6.3 bruit | PASS_BY_CONSTRUCTION | N/A |
| §6.4 𝔊^ero | PASS (ratio 0.855) | N/A |

**Verdict global** :
- `ENGINE_GUARANTEE_STATUS = PASS`
- `REGIME_VIABILITY_STATUS = ABSORBING_BASIN_EMPIRICAL_B2`

**Détail §6.2 — test de perturbation bornée** :

Protocole : (i) B2 stationnaire (7/125 cellules collapsées à β=60),
(ii) perturbation ψ injectée ε_ψ = 0.01 (gaussienne σ_pert=1.0 centrée
sur cellule la plus profonde), (iii) simulation 100 unités post-perturbation.

Résultat empirique :
- ψ_mass dans zone collapsée post-perturbation : 0.24 (ACCESS_CONFIRMED)
- β·⟨ψ⟩/γ dans zone target : **2.08** (sur-critique local)
- h_max post-perturbation dans zones collapsées : 6.3e-218
- 0/7 cellules remontent au-dessus de h_resolution

**Classification §6.2** : `ACCESS_NO_REACTIVATION_ABSORBING_BASIN_DETECTED`

**Mécanisme bifurcationnel local confirmé** (cohérent 4a-η) :

```
dh/dt ≈ h · (γ - β·ψ_local) ≈ h · (1 - 2.08) ≈ -1.08·h
```

Même avec perturbation injectée, ψ_local atteint dans la zone target
augmente le ratio β·ψ_local/γ au-dessus de 1, donc l'érosion logistique
ne peut pas redresser h.

**Correction mécanique importante** (audit Alex) :

> Une perturbation plus grande ne réactive pas nécessairement. Si elle
> augmente ψ_local, elle augmente aussi β·ψ_local/γ et peut renforcer
> le verrouillage. La condition de réactivation n'est pas "plus de
> perturbation" mais : β·ψ_local/γ < 1, ou introduction d'un mécanisme
> qui restaure h indépendamment de ψ_local (bruit, h_min explicite,
> érosion non proportionnelle à h, g_Ω, drift redistributif).

Une perturbation différente pourrait réactiver si elle **réduit** la
charge locale ψ dans la zone collapsée (par dilution sur volume plus
large), **redistribue** la masse, ou introduit une **composante de
restauration non proportionnelle à h**. Une perturbation concentrée
plus forte peut au contraire renforcer l'absorption.

**Caveat strict (à conserver verbatim)** :

> Empirically absorbing under ε_ψ = 0.01, t_post = 100, deterministic
> minimal dynamics. **NOT structurally absorbing in the full MCQ sense.**

L'absorption observée est dans la fenêtre simulée. Trois mécanismes
peuvent encore réactiver et n'ont pas été testés :
- (i) perturbation différente (plus diffuse, redistributive)
- (ii) bruit (stochastique)
- (iii) couplage inter-modulaire 𝒞^N (non encore implémenté)

**Conséquence structurelle inscrite** :

> **Validité du moteur ≠ viabilité du régime.**
>
> Le système produit un **KNV local** (cellules dynamiquement
> verrouillées) tout en restant numériquement cohérent (intégrité
> moteur préservée). C'est précisément le type de divergence productive
> utile pour préparer 6d-β, RR/RR², 𝒢 et 𝕋(t).

**Tension à conserver pour 6d-β** :

> Absorption empirique locale ≠ impossibilité générale de réactivation.

Cette tension doit rester ouverte jusqu'à ce que 6d-β teste les
observables non-locales (couplages, drift, bruit conservatif) qui
peuvent ou non sortir le système du bassin B2.
...
### 10.5 Verdict provisoire post-§5.8 / §6.6

Le verdict §10.1 initial supposait que §5 fonctionne comme test pivot
fort de la co-production MCQ. Les acquis empiriques §5.8 et §6.6 ont
modifié ce statut. Le verdict provisoire actuel doit refléter cette
évolution.

**Acquis du programme 6d-α (état à date)** :

| Étape | Statut | Lecture |
|---|---|---|
| §2.1 Diffusion pure | ✓ PASS | h uniforme + variable, machine precision |
| §2.2 Drift pur | ✓ PASS | 4 cas, conservation col_sums=0 |
| §2.3 OU stationnaire | ✓ PASS | CFL combinée découverte |
| §3a Diffusion + bruit | ✓ PASS | 3 σ_η, critère statistique diff/SEM<3 |
| §3a-bis Cartographie | ✓ CARACT | σ_η domine dt à D fixé |
| §3b OU + bruit | ✓ PASS STRUCTUREL | Drift déplace frontière positivité |
| §4a-α à 4a-η | ✓ PASS | Mémoire morphologique, feedback ψ↔h, STRATIFIED_REACTIVATION, mécanisme bifurcationnel local |
| §A.5 Calibration | ✓ TAUTOLOGIE | Identité algébrique au point fixe |
| §5.7 Famille B | ✓ MULTI-BASSINS | Bassin uniforme + bassin collapse partiel |
| §6 Garanties | ✓ ENGINE_PASS / VIABILITY=ABSORBING_BASIN_EMPIRICAL_B2 | Validité moteur ≠ viabilité régime |

**Verdict provisoire 6d-α** :
...
**Ce que 6d-α a validé** :
- Moteur 6d numériquement intègre (4 garanties préservées par construction)
- Co-évolution ψ↔h produit mémoire morphologique étendue empirique
- Stratification accès / réactivabilité / transformabilité (4a-η)
- Multi-bassins empiriques (§5.7)
- Bassin absorbant empirique sous perturbation 1% (§6.2)
- Distinction "validité moteur" / "viabilité régime"

**Ce que 6d-α n'a PAS validé** :
- Que §5 soit un pivot MCQ fort (il est algébriquement tautologique)
- Que les bassins observés soient structurellement absorbants au sens MCQ
- Que la co-production observée soit non-réductible à une dynamique
  ODE+PDE classique (la formule locale est trivialement satisfaite)
- Que τ', 𝒞_T, RR/RR² soient mesurables comme observables émergentes
  (test reporté à 6d-β)

**Caveat ontologique strict (à conserver verbatim)** :

> Le programme 6d-α valide la **cohérence numérique du moteur** et
> révèle des **régularités empiriques productives** (multi-bassins,
> mémoire morphologique, stratification réactivation). Il ne valide
> **pas** une co-production MCQ structurelle au sens Ch3 strict.
>
> La phrase §5.6 reste applicable :
>
...
> garantis MCQ".
>
> "Bassin empiriquement absorbant" ≠ "bassin structurellement absorbant".

**Tension productive à conserver pour 6d-β** :

> Validité du moteur ≠ viabilité du régime.
> Absorption empirique locale ≠ impossibilité générale de réactivation.
> Le système produit des KNV locaux tout en restant numériquement
> cohérent — c'est précisément la divergence productive utile pour
> 6d-β (observables non-locales RR/RR², 𝒢, 𝕋(t), MCQᴺ).

**Passage à 6d-β** :

Le passage à 6d-β est **autorisé** mais avec **statut conditionnel** :
les observables 6d-β (τ', 𝒞_T, β_QMC, RR/RR², 𝒢, 𝕋(t)) doivent
être interprétées **dans le régime du bassin atteint**, et leur
robustesse hors-distribution doit être testée comme en §5.7. Toute
conclusion MCQ forte 6d-β doit citer "co-production validée
hors-distribution dans bassin spécifié" et non "co-production
validée".

**Tests reportés à 6d-β ou plus tard** :
- Sweep ε_ψ pour caractériser le seuil de réactivation
- Test stochastique du bassin absorbant (bruit comme mécanisme
  réactivateur indépendant de ψ_local)
- Test couplage inter-modulaire 𝒞^N (sortir le système du bassin
  par mécanisme non-local)
- Test drift redistributif
