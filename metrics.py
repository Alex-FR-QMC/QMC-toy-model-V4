...
def synthesize_mediation(results_per_mode: dict) -> dict:
    ...
    # FULL is below the PASS threshold but ALPHA_ONLY passes — β attenuates α
    damped_pattern = (
        rel_alpha >= 0.05 and          # alpha alone clearly mediates
        rel_full < 0.05 and             # FULL does NOT pass (below threshold)
        rel_alpha > rel_full and        # full < alpha (something attenuates)
        rel_full > max_beta             # full > beta (alpha contributes to full)
    )
    # NB: this is the empirically observed Phase 6a pattern. With the
    # corrected sign of Φ_corr (positive bump at mean) and the corrected
    # divergence convention in _drift, the regulation term opposes the
    # diffusion's exposure of h_M's effect: β pushes mass outward via Φ_corr
    # which is already what diffusion is doing, but in a way that masks
    # the differential sensitivity to h_M. This produces FULL < ALPHA_ONLY.

    if full_ok:
        if alpha_ok and any_beta_ok:
            verdict = 'M_MEDIATES_VIA_BOTH'
        elif alpha_ok and not any_beta_ok:
            # FULL passes AND alpha passes, but no β-mode passes
            # → α dominant, β attenuates (FULL < ALPHA but still passes)
            if rel_full < rel_alpha - 0.01:
                verdict = 'M_MEDIATES_VIA_ALPHA_BETA_ATTENUATES'
            else:
    ...
    interpretations = {
        ...
        'M_MEDIATES_VIA_ALPHA_BETA_ATTENUATES': (
            "α (geometric diffusion) is the sole mediator of h_M on τ'. "
            "β (Φ_corr regulation) does NOT contribute independently — it does "
            "not pass F2' alone in any β-mode (BETA_ISOTROPIC, BETA_PURE). "
            "Moreover, β ATTENUATES α when both are active in FULL: "
            "rel_diff(FULL) < rel_diff(ALPHA_ONLY). Reading: the regulation "
            "drift overlaps with the diffusion's outward push and masks the "
            "differential sensitivity of τ' to h_M's evolution. β is not "
            "antagonistic in the strong sense — it does not invert the effect — "
            "but it numerically dampens what α alone would expose."
        ),
        'M_MEDIATES_VIA_ALPHA_DAMPED_BY_BETA': (
            "DEPRECATED: see M_MEDIATES_VIA_ALPHA_BETA_ATTENUATES. "
            "Kept for backward compatibility with earlier Phase 6a runs."
        ),
