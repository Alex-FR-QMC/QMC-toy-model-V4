...
# ============================================================================
# Δ_shape — variance-spread morphology measure
# ============================================================================

def compute_delta_shape(state: FactorialState) -> dict:
    """
    Δ_shape: morphological dispersion across the three axes via the
    standard deviation of (var_T, var_M, var_I).

    Complementary to compute_delta_modular (which measures spread of
    centred MEANS). For T25'-style contraction tests where the
    perturbation forces var_M without shifting E_M, the centred-mean
    Δ stays near zero while var_M changes substantially. Δ_shape is
    then the structurally meaningful indicator.

    Returns:
        delta_shape: std(var_T, var_M, var_I)
        var_T, var_M, var_I: per-axis marginal variances
        in_corridor: whether Δ_shape is in (delta_min, delta_crit) — same
                     thresholds as the centred Δ corridor (provisional;
                     a Δ_shape-specific corridor would require Ch.2
                     calibration).
    """
    psi = state.psi
    psi_T_marg = psi.sum(axis=(1, 2))
    psi_M_marg = psi.sum(axis=(0, 2))
    psi_I_marg = psi.sum(axis=(0, 1))

    E_T = float((THETA_T * psi_T_marg).sum())
    # Use M-centred coordinate so var is around the same axis value
    E_M_raw = float((THETA_M * psi_M_marg).sum())
    E_I = float((THETA_I * psi_I_marg).sum())

    var_T = float((((THETA_T - E_T) ** 2) * psi_T_marg).sum())
    var_M = float((((THETA_M - E_M_raw) ** 2) * psi_M_marg).sum())
    var_I = float((((THETA_I - E_I) ** 2) * psi_I_marg).sum())

    delta_shape = float(np.std([var_T, var_M, var_I]))
    in_corridor = bool(
        KNV_THRESHOLDS_6C_B['delta_min'] < delta_shape
...
def compute_gamma_signals(history: dict, dt: float) -> dict:
    """
    Multi-signal Γ″ proxy (Alex's correction for T25'/T27').

    For a forced contraction test, Γ″ on a single τ′ component (e.g. tau_T)
    can miss the morphological dynamics being forced. We compute Γ″ on
    THREE complementary signals and require at least 2 to be non-dampable
    for morph_KNV.

    Inputs (history is a dict with arrays of length n_steps):
      - 'var_M': per-step var_M of the target module
      - 'h_M_mean': per-step mean of h_M of the target module
      - 'tau_vec': per-step (tau_T, tau_I) of the target module — shape (n, 2)

    For each signal we compute Γ″ via second finite difference and check:
      - growth_ratio = max|Γ″| in 2nd half / max|Γ″| in 1st half
      - normalised_growth = growth_ratio after dividing each half-max by
        the |signal| amplitude in that half (catches "Γ″ grows because
        signal grows", not because dynamics is unstable)
      - dampable iff normalised_growth_ratio < threshold (1.5 by default)

    Returns per-signal damp diagnostics + aggregate non_dampable_count.
    """
    out = {}
    for key in ['var_M', 'h_M_mean']:
        arr = np.asarray(history.get(key, []), dtype=float)
        if arr.size < 4 or not np.all(np.isfinite(arr)):
            out[key] = {'dampable': True, 'reason': 'INSUFFICIENT_OR_NAN'}
            continue
        gamma = (arr[2:] - 2 * arr[1:-1] + arr[:-2]) / (dt * dt)
        n = len(gamma)
        if n < 4:
            out[key] = {'dampable': True, 'reason': 'TOO_SHORT'}
            continue
        first, second = gamma[:n // 2], gamma[n // 2:]
        amp_first = float(np.abs(first).max())
        amp_second = float(np.abs(second).max())
        # Normalise by |signal| amplitude in each half
        sig_first = float(np.abs(arr[:n // 2 + 1]).max()) + 1e-12
        sig_second = float(np.abs(arr[n // 2 + 1:]).max()) + 1e-12
        norm_first = amp_first / sig_first
        norm_second = amp_second / sig_second
        norm_growth = norm_second / max(norm_first, 1e-12)
        dampable = (norm_growth < 1.5)
        out[key] = {
            'dampable': dampable,
            'gamma_max': float(np.abs(gamma).max()),
            'amp_first_half': amp_first,
            'amp_second_half': amp_second,
            'normalised_growth': norm_growth,
        }
    # tau_vec: 2D
    tau_vec = np.asarray(history.get('tau_vec', []), dtype=float)
    if tau_vec.ndim == 2 and tau_vec.shape[0] >= 4 and np.all(np.isfinite(tau_vec)):
        gamma_vec = (tau_vec[2:] - 2 * tau_vec[1:-1] + tau_vec[:-2]) / (dt * dt)
        gamma_norms = np.linalg.norm(gamma_vec, axis=1)
        n = len(gamma_norms)
        first, second = gamma_norms[:n // 2], gamma_norms[n // 2:]
        amp_first = float(first.max())
        amp_second = float(second.max())
        sig_first = float(np.linalg.norm(tau_vec[:n // 2 + 1], axis=1).max()) + 1e-12
        sig_second = float(np.linalg.norm(tau_vec[n // 2 + 1:], axis=1).max()) + 1e-12
        norm_first = amp_first / sig_first
        norm_second = amp_second / sig_second
        norm_growth = norm_second / max(norm_first, 1e-12)
        dampable = (norm_growth < 1.5)
        out['tau_vec'] = {
            'dampable': dampable,
            'gamma_max': float(gamma_norms.max()),
            'amp_first_half': amp_first,
            'amp_second_half': amp_second,
            'normalised_growth': norm_growth,
        }
    else:
        out['tau_vec'] = {'dampable': True, 'reason': 'INSUFFICIENT_OR_NAN'}

    n_non_dampable = sum(1 for k, v in out.items() if not v.get('dampable', True))
    aggregate_morph_KNV = (n_non_dampable >= 2)

    return {
        'per_signal': out,
        'n_non_dampable_signals': n_non_dampable,
        'aggregate_morph_KNV': aggregate_morph_KNV,
        'note': (
            "morph_KNV requires ≥ 2 of {var_M, h_M_mean, tau_vec} signals "
            "to show normalised Γ″ growth ratio ≥ 1.5. Single-signal "
            "non-damping is insufficient to declare morphogenic KNV."
        ),
    }
