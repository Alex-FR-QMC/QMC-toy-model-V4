...
| `BETA_ISOTROPIC` | isotropic (h=h_0) | active | active | ✗ instable* |
| `BETA_PURE` | OFF | active | OFF | ✗ instable* |
| `NO_REGULATION_BASELINE` | isotropic (h=h_0) | OFF | active | ✓ |

\* See "BETA modes instability" below.
...
The first implementation of Φ_corr had an inverted sign in the anti-collapse
term. The bump was constructed as a *negative well* at the mean (Phi -= ...),
which under drift = -∇·(ψ·∇Φ) produced an *attractive* force toward the mean,
not the intended repulsion. This was pro-collapse, not anti-collapse.

The bug was masked by the fact that the inverted regulation, being weaker
in net effect, did not destabilise the explicit Euler integration.
Corrected: Phi_corr is now built as a *positive bump* at the mean
(Phi += ...), so -∇Phi points outward.
...
## BETA modes instability

`BETA_ISOTROPIC` and `BETA_PURE` produce mass drift that does NOT vanish
in dt → 0 limit. Scan at constant physical time T_phys = 10:

| dt | T_steps | drift (BETA_ISOTROPIC) | drift (BETA_PURE) |
|----|---------|------------------------|--------------------|
| 0.050 | 200 | 1.64e-01 | 1.69e+00 |
| 0.025 | 400 | 1.64e-01 | 1.69e+00 |
| 0.010 | 1000 | 1.64e-01 | 1.69e+00 |
| 0.005 | 2000 | 1.64e-01 | 1.69e+00 |
| 0.002 | 5000 | 1.64e-01 | 1.69e+00 |
| 0.001 | 10000 | 1.64e-01 | 1.69e+00 |

The drift converges to a finite non-zero value. This is **not a discretisation
artefact** — it is a property of the continuous-time dynamics in these modes.

**Reading (capacity result, not theorem):** under the present form of
Φ_corr (Gaussian repulsive bumps on a domain with reflective boundaries
and λ_KNV = 0.05), the regulation drift pushes mass toward the boundary
faster than a noise-only or isotropic-only diffusion can replenish it. The
clipping at the boundary then consumes mass irreducibly.

This is **specific to** this Φ_corr form (Gaussian bumps that don't vanish
at boundaries) and to the absence of a sufficient compensating diffusion
mechanism. A Φ_corr that vanishes at the domain boundary, or a stronger
diffusion floor, would likely change the result. The finding is real but
its scope is limited.

## Status (Phase 6a, after corrections)

| Test | FULL | ALPHA_ONLY | BETA_ISOTROPIC | BETA_PURE | NO_REG_BASELINE |
|------|------|------------|-----------------|-----------|-----------------|
| Invariants | ✓ | ✓ | INVALID | INVALID | ✓ |
| F2 | PASS | PASS | n/a | n/a | PASS borderline |
| F2' | PASS 15.0% | PASS 10.0% | n/a | n/a | 0.0% (confirms h-in-α is essential) |
| F3a | INVISIBLE_M (by construction) | INVISIBLE_M | n/a | n/a | INVISIBLE_M |
| F3b on M | WEAK_DIVERGENCE 3.3% | WEAK_DIVERGENCE 3.0% | n/a | n/a | WEAK_DIVERGENCE 4.0% |
| MCF intra | T=2000: 0.01% ✓ | n/a | n/a | n/a | n/a |

## Synthesis verdict

**M_MEDIATES_VIA_BOTH** (cooperative): α and β both contribute to h_M's
effect on τ'. β amplifies α (FULL > ALPHA_ONLY by 5%), not antagonist.

The pre-correction reading "β dampens α" was an artefact of the Φ_corr
sign error. With the corrected sign, regulation cooperates with the
geometric diffusion to expose h_M's mediation.
