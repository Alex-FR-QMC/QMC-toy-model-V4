...
class FactorialEngine:
    ...
    def _drift(
        ...
        Sign convention note:
          The flux J_drift = -ψ · ∇Φ is the physical convention (current
          drift speed = -∇Φ). The continuity equation gives:
              ∂_t ψ |_drift = -∇·J_drift
          To compute -∇·J on the discrete grid, we use:
              -div = -(flux[i+1] - flux[i]) = flux[i] - flux[i+1]
          This is OPPOSITE to the diffusion code, where the flux convention
          itself was already opposite (J_diff = +D·∇ψ instead of -D·∇ψ),
          so the two sign flips compensated. Here we keep the physical
          convention for J_drift, so we return -(flux[i+1] - flux[i]).

        Bug history (2026-04): an earlier version returned (flux[i+1] - flux[i])
          which silently inverted the drift, making Φ_corr act pro-collapse
          instead of anti-collapse. Detected by micro-test (init concentré
          + var<var_min: drift returned positive at centre, attracting more
          mass, hence pro-collapse).
        """
