...
def _compute_novelty_and_form(
    ...
    if form == 'perspectival_MORPHO_ACTIVE':
        ...
        elif isinstance(prev_h_field, list):
            # Windowed: prev_h_field is a list of the last k h_fields,
            # ordered from oldest to most recent. Compute |∂_t h|_eff
            # as the mean of |h(t_i) - h(t_{i-1})| over the window.
            # This smooths out single-step spikes (per Alex's correction).
            if len(prev_h_field) == 0:
                dh_dt_abs = np.zeros_like(h_field)
            else:
                # Build sequence: prev_h_field[0], ..., prev_h_field[-1], h_field(current)
                seq = prev_h_field + [h_field]
                # Compute step-to-step absolute differences and mean them
                diffs = [np.abs(seq[i] - seq[i-1]) / max(dt, 1e-12)
                         for i in range(1, len(seq))]
                dh_dt_abs = np.mean(diffs, axis=0)
        else:
            # Backward compat: single previous h_field (1-step difference)
...
def step_three_modules(
    ...
    # Snapshot the current h_fields (BEFORE step) as part of the sliding
    # window for next step. MORPHO_ACTIVE will compute |∂_t h|_eff as
    # the mean of step-to-step differences over the last k steps.
    MORPHO_ACTIVE_WINDOW_K = 5
    current_h_fields = {
        name: _h_field_product(states[name])  # snapshot pre-step
        for name in ['A', 'B', 'C']
    }

    # If prev_h_fields was a list (windowed), append; else convert to list
    new_prev_h_fields = {}
    for name in ['A', 'B', 'C']:
        if isinstance(prev_h_fields, dict) and name in prev_h_fields:
            existing = prev_h_fields[name]
            if isinstance(existing, list):
                # Already windowed: append and trim
                new_prev_h_fields[name] = (existing + [current_h_fields[name]])[-MORPHO_ACTIVE_WINDOW_K:]
            else:
                # Was single-step: start window with the existing prev + current
                new_prev_h_fields[name] = [existing, current_h_fields[name]][-MORPHO_ACTIVE_WINDOW_K:]
        else:
            # First step: window starts with just the current
            new_prev_h_fields[name] = [current_h_fields[name]]
