...
from mcq_v4.factorial.metrics import (
    ...
    metric_F3b_morphodynamic, metric_MCF_intra, synthesize_mediation,
)
...
def run_phase_6a() -> dict:
    ...
    base_engine_cfg_kwargs = dict(
        ...
        H_min=0.5, lambda_KNV=0.05,  # calibrated to keep FULL mode numerically clean
    )
    ...
    all_modes = [
        EngineMode.FULL,
        EngineMode.ALPHA_ONLY,
        EngineMode.BETA_ISOTROPIC,
        EngineMode.BETA_PURE,
        EngineMode.NO_REGULATION_BASELINE,
    ]
    for mode in all_modes:
        ...
        f3a = metric_F3_temporal(history_free, sample_every=10)

        # F3b — internal morphodynamic transformability (perturb on M axis)
        # Build a fresh engine factory bound to this mode's config
        cfg_mode_local = cfg_mode  # capture in closure
        mcfg_local = module_cfg
        from mcq_v4.factorial.engine import FactorialEngine as _Engine
        engine_factory = lambda c=cfg_mode_local, m=mcfg_local: _Engine(c, m)
        initial_state = make_initial_state(psi_init, module_cfg)
        try:
            f3b = metric_F3b_morphodynamic(
                engine_factory, initial_state,
                horizon=50, perturb_axis='M', perturb_eps=0.05,
                perturb_at_step=100,
            )
        except Exception as e:
            f3b = {'error': str(e), 'outcome': 'ERROR'}
    ...
    # ----- MCF intra (only FULL mode), with proper convergence test -----
    # Same module config (same seed), different initial conditions.
    # Multi-seed robustness can be tested separately.
    # T_steps=2000 is the empirically validated horizon for clean MCF convergence
    # (max relative diff ~ 0.01% — three orders below the 5% threshold).
    cfg_full = FactorialEngineConfig(**base_engine_cfg_kwargs, mode=EngineMode.FULL)
    cfg_long = FactorialEngineConfig(**{**base_engine_cfg_kwargs, 'T_steps': 2000},
                                     mode=EngineMode.FULL)
    psi_alt = init_psi_uniform()

    # Same module config (same seed) for both runs to isolate IC effect from noise effect
    history_mcf_1, diag_mcf_1 = run_simulation(cfg_long, module_cfg, psi_init)
    history_mcf_2, diag_mcf_2 = run_simulation(cfg_long, module_cfg, psi_alt)

    inv_mcf_1 = metric_invariants(history_mcf_1, diag_mcf_1, cfg_long)
    inv_mcf_2 = metric_invariants(history_mcf_2, diag_mcf_2, cfg_long)
...
def print_phase_6a_report(verdict: dict):
    ...
    for mode_name, r in verdict['modes'].items():
        ...
        # F3a (visible polymorphism via τ')
        f3a = r['F3a']
        print(f"  F3a (tensional-visible polymorphism): {f3a['outcome']} ({f3a['signal_quality']})")
        print(f"    cardinality: min = {f3a['min_cardinality']}, "
              f"max = {f3a['max_cardinality']}")
        print(f"    identity changes: {f3a['identity_changes']}")
        print(f"    NOTE: |𝒟| ≤ 4 by construction (M invisible to τ')")

        # F3b (internal morphodynamic transformability)
        f3b = r.get('F3b_M', {})
        if f3b.get('outcome') == 'ERROR':
            print(f"  F3b (M-perturb internal): ERROR — {f3b.get('error', '?')}")
        else:
            print(f"  F3b (M-perturb internal): {f3b['outcome']} ({f3b['signal_quality']})")
            print(f"    div_var_M: {f3b['div_var_M']:.4e}, "
                  f"div_psi_max: {f3b['div_psi_max']:.4e}")
