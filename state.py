...
class EngineMode(Enum):
    ...
    FULL                     : α + β. Geometric diffusion (h-modulated) + Φ_corr regulation.
    ...
    BETA_ISOTROPIC           : β + isotropic diffusion. Φ_corr active, but the
                               diffusion uses h_0 instead of h on each axis (isotropic).
                               Tests whether the regulatory loop produces tripartition
                               under a neutral diffusion (decouples the geometric
                               diffusion structure from the regulation).
                               (This was the previous BETA_ONLY mode.)
    BETA_PURE                : β only. Φ_corr active, NO diffusion at all (D_eff = 0
                               in the diffusion term). Noise is also disabled in this
                               mode for diagnostic clarity. Tests the proper effect
                               of the regulatory drift in isolation.
    NO_REGULATION_BASELINE   : Isotropic diffusion only. Φ_corr ≡ 0 AND h is replaced
                               by h_0 in diffusion. Baseline reference: what would
                               happen with neutral diffusion and no regulation
                               (free thermalisation).

    Aliases for backward compatibility:
      BETA_ONLY = BETA_ISOTROPIC (deprecated)
    """
    FULL = "FULL"
    ALPHA_ONLY = "ALPHA_ONLY"
    BETA_ISOTROPIC = "BETA_ISOTROPIC"
    BETA_PURE = "BETA_PURE"
    NO_REGULATION_BASELINE = "NO_REGULATION_BASELINE"
    # Backward compatibility (old name)
    BETA_ONLY = "BETA_ISOTROPIC"
...
class FactorialEngineConfig:
    ...
    var_max: float = 2.5
    H_min: float = 0.5
    lambda_KNV: float = 0.05  # recalibrated 2026-04: was 0.3 (too strong with corrected
                              # sign of Φ_corr). At 0.05, full mode runs numerically clean
                              # (mass_drift ~ 1e-16, 0 clips). Higher values produce
                              # NUMERICAL_INVALID through diffusive divergence of the
                              # repulsive bumps under the explicit Euler scheme.
