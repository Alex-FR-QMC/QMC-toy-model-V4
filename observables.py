...
def compute_Phi_corr(
    ...
    In BETA_ONLY mode, Φ_corr is computed normally (regulation kept).
    In FULL mode, Φ_corr is computed normally.
    """
    if mode == EngineMode.ALPHA_ONLY:
    ...
    for var_a, mean_a, theta_grid in [
        ...
        if var_a < cfg.var_min:
            # Anti-collapse: NEGATIVE bump at mean → repulsive force
            # (-∇Φ points away from mean where Φ is most negative)
