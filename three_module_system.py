...
class CouplingConfig:
    ...
    coupling_form: str = "contrastive"
    """
    Which coupling form to use. Two choices, both non-perspectival:

      'positive'    : phi_extra_i += ε · R_ij(1-R_ij) · ψ_j
                      Numerically repulsive (mass flees Φ peaks, which
                      coincide with ψ_j's peaks). Tested by the coupling
                      sign micro-test as COUPLING_REPULSIVE.

      'contrastive' : phi_extra_i += ε · R_ij(1-R_ij) · (ψ_j - R_ij · ψ_i)
                      Closer to the toy V2 §2.2 formula. Subtracts the
                      'already-known to self' component, keeping only
                      relative novelty. Also COUPLING_REPULSIVE in the
                      sign micro-test.

    Default is 'contrastive' as it is closer to the architectural intent
    of toy V2 (transmits novelty, not absolute position).

    Conceptual reading
    ------------------
    Both coupling forms are NUMERICALLY repulsive: in the isolation
    micro-test (test_coupling_sign.py), module i's centre of mass moves
    AWAY from module j's centre of mass under a single coupling step.

    But the right architectural reading is NOT 'i flees j's novelty'.
    The right reading is:

      Inter-modular novelty is CONVERTED INTO a gradient of
      differentiation.

    The system does not yet integrate j's novelty as common identity;
    it first transforms that novelty into morphodynamic differentiation.
    The repulsion observed in the micro-test is the numerical signature
    of this differentiation operation. R(1-R) ensures it vanishes at
    R→0 (nothing to differentiate from) and R→1 (modules already
    identical, no novelty to convert).

    Phenomenologically (HYPOTHETICAL — to confirm architecturally, not
    yet established): this differentiation might be the first stage of
    emergence of a distributed identity not yet interpretable by the
    system itself. Phase 6b does NOT claim to validate this — it
    establishes the perturbative coupling baseline on which Phase 6c
    will build the perspective-dependent native coupling.
    """
...
class ThreeModuleSystem:
    ...
    # Previous h_field per module — tracked across steps for the
    # MORPHO_ACTIVE perspectival coupling (which uses |∂_t h|).
    # None for the first step; populated after each call to
    # step_three_modules. Backward-compatible: forms that don't need it
    # ignore this field.
    ...
    def replace_states(self, new_states: dict[str, FactorialState],
                       new_prev_h_fields: Optional[dict] = None) -> 'ThreeModuleSystem':
        ...
        return ThreeModuleSystem(
            ...
            prev_h_fields=new_prev_h_fields if new_prev_h_fields is not None
                          else self.prev_h_fields,
        )
