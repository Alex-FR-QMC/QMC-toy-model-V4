...
### 2.1 Cas 1 — Diffusion pure isotrope homogène (corrections b, c)
...
Solution analytique (continuum) : ψ(θ, t) = (4πD_eff·t')^{-3/2}·
exp(-‖θ-θ_0‖²/(4D_eff·t')) avec t' = t + σ_0²/(2D_eff) (offset initial).

**Test** :
- À t = 5·dx²/D_eff après initialisation, mesurer la **variance par
  axe** : Var_axis_obs (correction c — par axe, pas total).
- Comparer à Var_axis_pred = 2·D_eff·(t + σ_0²/(2D_eff)) = 2·D_eff·t + σ_0².
- Tolérance : 5% sur la variance par axe.
- Vérifier les 3 axes : x, y, z doivent donner Var identique à 5% près
  (test isotropie léger ici, vrai test §3).

**Si échec** : bug schéma diffusion ou problème d'interpolation initiale.
