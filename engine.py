...
with the following currents:
  ...
  J_drift  = -ψ · ∇Φ_corr                     (mass flows from high Φ to low Φ)
  J_noise  =  σ · D_eff · sqrt(ψ_L · ψ_R) · ξ / h_a(edge)   (multiplicative flux)

Substituting:

  ∂_t ψ = ∇·(D_eff/h^2 · ∇ψ)        (α: diffusion in geometry)
        + ∇·(ψ · ∇Φ_corr)            (β: regulation; SIGN: positive bump → outward push)
        - ∇·J_noise                  (multiplicative flux noise)

(Note the +sign on the regulation term: it follows from J_drift = -ψ ∇Φ
and ∂_t ψ = -∇·J. A positive bump at the mean of Φ_corr drives mass
outward — anti-collapse. See _drift docstring for full derivation.)
...
The engine supports five modes:
  FULL                    — α + β + noise (canonical)
  ALPHA_ONLY              — diffusion + noise, Φ_corr ≡ 0
  BETA_ISOTROPIC          — isotropic diffusion (h=h_0) + Φ_corr + noise
  BETA_PURE               — Φ_corr only (no diffusion, no noise)
  NO_REGULATION_BASELINE  — isotropic diffusion + noise, Φ_corr ≡ 0
"""
...
class FactorialEngine:
    ...
    def _diffuse(
        ...
        Mode-dependent behaviour:
          FULL                    : full geometric diffusion (h-modulated)
          ALPHA_ONLY              : full geometric diffusion (h-modulated)
          BETA_ISOTROPIC          : isotropic diffusion (h replaced by h_0)
          BETA_PURE               : no diffusion (return zeros)
          NO_REGULATION_BASELINE  : isotropic diffusion (h replaced by h_0)
        """
        # BETA_PURE: no diffusion at all
        if mode == EngineMode.BETA_PURE:
            return np.zeros_like(psi)

        # In BETA_ISOTROPIC and NO_REGULATION_BASELINE modes, use isotropic baseline h
        if mode in (EngineMode.BETA_ISOTROPIC, EngineMode.NO_REGULATION_BASELINE):
    ...
    def _drift(
        ...
        """
        Regulatory drift contribution to ∂_t ψ.

        Mathematical content
        --------------------
        We model the drift current J = -ψ · ∇Φ_corr (physical convention:
        drift speed = -∇Φ, so mass flows from high Φ toward low Φ).
        The continuity equation gives:

            ∂_t ψ |_drift = -∇·J = -∇·(-ψ ∇Φ) = +∇·(ψ ∇Φ)

        So this function returns +∇·(ψ ∇Φ), NOT -∇·(ψ ∇Φ).

        The earlier docstring described the term as "-∇·(ψ · ∇Φ_corr)",
        which is incorrect (off by a sign). The intended dynamics is
        anti-collapse when Φ has a positive bump at the mean (var < var_min):
        the bump produces ∇Φ pointing outward from the mean (positive on
        the side beyond the peak, negative on the near side), so the drift
        speed v = -∇Φ points AWAY from the mean. Mass flows outward, hence
        anti-collapse. Verified by micro-test in
        tests/phase6a/test_sign_regression.py.

        Geometric weighting
        -------------------
        Gradient on each axis: ∂_a Φ / h_a(edge), where h_a(edge) is the
        edge-averaged metric. ψ is also edge-averaged for the flux.

        Discretisation
        --------------
        Interior edges only; reflective (zero-flux) boundary on all axes.
        The discrete divergence operator differs from `_diffuse` by an
        explicit sign because here the flux J already carries the physical
        minus-sign in J = -ψ ∇Φ. See git history 2026-04 for the fix.
        """
        ...
        # Drift fluxes (physical convention): J = -ψ · ∇Φ
        ...
        # Continuity ∂_t ψ = -∇·J → return flux[i] - flux[i+1] = -(flux[i+1] - flux[i])
    ...
    def _noise_term(
        ...
        Mode-dependent behaviour:
          BETA_PURE                    : noise disabled (return zeros)
          BETA_ISOTROPIC               : isotropic h (replaced by h_0)
          NO_REGULATION_BASELINE       : isotropic h (replaced by h_0)
          FULL, ALPHA_ONLY             : full h modulation
        """
        # BETA_PURE: no noise (deterministic regulatory drift only)
        if mode == EngineMode.BETA_PURE:
            return np.zeros_like(psi)
    ...
    def step(
        ...
        phi_extra: Optional[np.ndarray] = None,
        ...
        Order:
          ...
          4. (Phase 6b only) If phi_extra is not None, add it to Φ_corr
          5. Compute ∂ψ/∂t = diffusion + drift + noise
          6. Update ψ with explicit Euler
          7. Clip to enforce positivity (no renormalisation — log clips)
          8. Update marginal metric (skip h_M if frozen)

        Phase 6b extension
        ------------------
        The optional `phi_extra` argument is the inter-modular coupling
        contribution to Φ_corr, computed externally from neighbour modules
        in Phase 6b. When phi_extra is None (the default), this method
        is BITWISE-IDENTICAL to the Phase 6a engine (no other code paths
        change). This is verified by the non-regression test
        `test_phi_extra_none_regression.py`.
        ...
        # 3. Φ_corr (intra-modular regulation)
        Phi_corr = compute_Phi_corr(obs, self.cfg, mode)

        # 4. Optional inter-modular coupling extra (Phase 6b)
        if phi_extra is not None:
            Phi_corr = Phi_corr + phi_extra
